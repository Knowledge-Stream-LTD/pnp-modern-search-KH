...
import {
  AvailableLayouts,
  BuiltinLayoutsKeys,
} from "../../layouts/AvailableLayouts";
import { LayoutHelper } from "../../helpers/LayoutHelper";
import { TemplateService } from "../../services/templateService/TemplateService";
import {
  FileFormat,
  ITemplateService,
} from "../../services/templateService/ITemplateService";
import {
  isEmpty,
  isEqual,
  uniqBy,
...
import {
  ...
  BuiltinFilterTypes,
} from "../../layouts/AvailableTemplates";
...
export default class SearchFiltersWebPart
  extends BaseWebPart<ISearchFiltersWebPartProps>
  implements IDynamicDataCallables
{
   ...
   */
  private availableLayoutDefinitions: ILayoutDefinition[] =
    AvailableLayouts.BuiltinLayouts.filter((layout) => {
      return layout.type === LayoutType.Filter;
    });
  ...
  private availableWebComponentDefinitions: IComponentDefinition<any>[] =
    AvailableComponents.BuiltinComponents;
  ...
  protected async onInit() {
    try {
      ...
      Log.warn(
        LogSource,
        `Opt out for PnP Telemetry failed. Details: ${error}`,
    ...
    await this.templateService.registerWebComponents(
  ...
  public async render(): Promise<void> {
    ...
    if (this.lastLayoutKey !== this.properties.selectedLayoutKey) {
      this.layout = await LayoutHelper.getLayoutInstance(
        this.webPartInstanceServiceScope,
        this.context,
        this.properties,
        this.properties.selectedLayoutKey,
  ...
  protected renderCompleted(): void {
    ...
    if (
      ...
      this.properties.filtersConfiguration.length > 0
    ) {
      ...
      if (this._dataSourceDynamicProperties.length > 0) {
        ...
        const dataResultsSourceData = this.mergeFiltersDataFromSources(
      ...
      filterResults = this._initStaticFilters(
        filterResults,
        this.properties.filtersConfiguration
      );

      renderRootElement = React.createElement(SearchFilters, {
      ...
      if (this.displayMode === DisplayMode.Edit) {
        ...
        Log.verbose(
          `[SearchResultsWebPart.renderCompleted]`,
          `The 'renderRootElement' was null during render.`,
          this.webPartInstanceServiceScope
        );
      }
    }
    ...
    if (
      this._verticalsSourceData &&
      this.properties.selectedVerticalKeys.length > 0
    ) {
      const verticalData = DynamicPropertyHelper.tryGetValueSafe(
        this._verticalsSourceData
      );
      ...
      if (
        ...
        this.properties.selectedVerticalKeys.indexOf(
          verticalData.selectedVertical.key
        ) === -1
      ) {
        if (this.displayMode === DisplayMode.Edit) {
          ...
          const verticalNames = verticalData.verticalsConfiguration
            .filter((cfg) => {
              return (
                this.properties.selectedVerticalKeys.indexOf(cfg.key) !== -1
              );
            })
            .map((v) => v.tabName);
          ...
          if (parentControlZone) {
            parentControlZone.setAttribute(
              "style",
    ...
    return [
      {
        ...
        title: this.properties.title
          ? `${this.properties.title} - ${this.instanceId}`
          : `${webPartStrings.General.WebPartDefaultTitle} - ${this.instanceId}`,
      },
    ];
  }
  ...
  public onCustomPropertyUpdate(
    ...
    if (propertyPath.localeCompare("selectedVerticalKeys") === 0) {
      changeCallback(
        propertyPath,
        (cloneDeep(newValue) as IComboBoxOption[]).map((v) => {
    ...
    propertyPanePages.push(
      {
        groups: [
          {
            groupName:
              commonStrings.PropertyPane.ConnectionsPage
                .DataConnectionsGroupName,
            groupFields: this.propertyPaneConnectionsFields,
          },
          {
            groupName:
              webPartStrings.PropertyPane.FiltersSettingsPage.SettingsGroupName,
  ...
  protected async onPropertyPaneFieldChanged(
    ...
    if (
      propertyPath.localeCompare("filtersConfiguration") === 0 &&
      !isEqual(oldValue, newValue)
    ) {
      // Remove duplicate fields. We can't have multiple filters with the same field
      this.properties.filtersConfiguration = uniqBy(
        this.properties.filtersConfiguration,
        "filterName"
      );
      ...
      this.properties.filtersConfiguration = (
        newValue as IDataFilterConfiguration[]
      ).map((configuration) => {
        if (
          configuration.selectedTemplate === BuiltinFilterTemplates.DateRange ||
    ...
    if (propertyPath.localeCompare("inlineTemplateContent") === 0) {
      ...
      if (
        this.properties.inlineTemplateContent &&
    ...
    if (
      propertyPath.localeCompare("selectedLayoutKey") === 0 &&
      !isEqual(oldValue, newValue) &&
      this.properties.selectedLayoutKey !==
        BuiltinLayoutsKeys.ResultsDebug.toString()
    ) {
    ...
    this.propertyPaneConnectionsFields =
      await this.getConnectionOptionsFields();
    this.context.propertyPane.refresh();
  }
  ...
  private async getDataResultsConnectionFields(): Promise<
    IPropertyPaneField<any>[]
  > {
    const availableSources =
      await this.dynamicDataService.getAvailableDataSourcesByType(
    ...
    sourceOptions.forEach((option) => {
      if (
        this.properties.dataResultsDataSourceReferences.indexOf(
    ...
    return [
      new PropertyPaneAsyncCombo("dataResultsDataSourceReferences", {
        ...
        label:
          webPartStrings.PropertyPane.ConnectionsPage
            .UseDataResultsFromComponentsLabel,
        description:
          webPartStrings.PropertyPane.ConnectionsPage
            .UseDataResultsFromComponentsDescription,
        onPropertyChange: (
          propertyPath,
          ...
          changeCallback(
            propertyPath,
            newValue.map((option) => option.key as string)
          );
          this.ensureDynamicDataSourcesConnection();
  ...
  private async getConnectionOptionsFields(): Promise<
    IPropertyPaneField<any>[]
  > {
    const dataResultsConnectionsFields =
      await this.getDataResultsConnectionFields();
  ...
  private getStylingPageGroups(): IPropertyPaneGroup[] {
    ...
    stylingFields.push(
      this._propertyFieldCodeEditor("inlineTemplateContent", {
        ...
        panelTitle:
          webPartStrings.PropertyPane.LayoutPage.FiltersTemplatePanelHeader,
    ...
    if (
      ...
      stylingFields.push(
        PropertyPaneTextField("externalTemplateUrl", {
          ...
          placeholder:
            webPartStrings.PropertyPane.LayoutPage.TemplateUrlPlaceholder,
          deferredValidationTime: 500,
    ...
    let groups: IPropertyPaneGroup[] = [
      {
        groupName:
          webPartStrings.PropertyPane.LayoutPage.AvailableLayoutsGroupName,
    ...
    if (layoutOptions.length > 0) {
      groups.push({
        groupName:
          webPartStrings.PropertyPane.LayoutPage.LayoutTemplateOptionsGroupName,
  ...
  private getFilterSettings(): IPropertyPaneField<any>[] {
    ...
    if (this._dataSourceDynamicProperties) {
      ...
      this._dataSourceDynamicProperties.forEach((dataSourceDynamicProperty) => {
        const dataResultsData = DynamicPropertyHelper.tryGetValueSafe(
          dataSourceDynamicProperty
        );
        if (dataResultsData) {
          allAvailableFieldsFromResults = allAvailableFieldsFromResults.concat(
            dataResultsData.availableFieldsFromResults
          );
        }
      });

      availableFieldOptionsFromResults = uniq(
        allAvailableFieldsFromResults
      ).map((field) => {
      ...
      filterNameCollectionField = [
        {
          id: "filterName",
          title:
            webPartStrings.PropertyPane.DataFilterCollection.FilterNameLabel,
          ...
          onCustomRender: (field, value, onUpdate, item) => {
            return React.createElement(
              ...
              React.createElement(AsyncCombo, {
                ...
                placeholder:
                  webPartStrings.PropertyPane.DataFilterCollection
                    .SelectFilterComboBoxLabel,
    ...
    let filterSettings = [
      this._propertyFieldCollectionData("filtersConfiguration", {
        manageBtnLabel:
          webPartStrings.PropertyPane.DataFilterCollection
            .CustomizeFiltersBtnLabel,
        key: "filters",
        enableSorting: true,
        panelHeader:
          webPartStrings.PropertyPane.DataFilterCollection
            .CustomizeFiltersHeader,
        panelDescription:
          webPartStrings.PropertyPane.DataFilterCollection
            .CustomizeFiltersDescription,
        label:
          webPartStrings.PropertyPane.DataFilterCollection
            .CustomizeFiltersFieldLabel,
        ...
        fields: [
          ...
          ...filterNameCollectionField,
          {
            id: "maxBuckets",
            title:
              webPartStrings.PropertyPane.DataFilterCollection.FilterMaxBuckets,
          ...
          },
          {
            id: "selectedTemplate",
            title:
              webPartStrings.PropertyPane.DataFilterCollection.FilterTemplate,
            type: this._customCollectionFieldType.dropdown,
            required: true,
            options: [
              {
                key: BuiltinFilterTemplates.CheckBox,
                text: webPartStrings.PropertyPane.DataFilterCollection.Templates
                  .CheckBoxTemplate,
              },
              {
              ...
              {
                key: BuiltinFilterTemplates.NumberRange,
                text: webPartStrings.PropertyPane.DataFilterCollection.Templates
                  .NumberRangeTemplate,
              },
            ],
            ...
            onCustomRender: (
              ...
              return React.createElement(
                ...
                React.createElement(Checkbox, {
                  defaultChecked:
                    item.selectedTemplate === BuiltinFilterTemplates.ComboBox
                      ? false
                      : item.expandByDefault,
                  disabled:
                    item.selectedTemplate === BuiltinFilterTemplates.ComboBox,
          ...
          },
          {
            ...
            onCustomRender: (
              ...
              return React.createElement(
                ...
                React.createElement(Checkbox, {
                  defaultChecked:
                    item.selectedTemplate === BuiltinFilterTemplates.DateRange
                      ? false
                      : item.showCount,
                  disabled:
                    item.selectedTemplate === BuiltinFilterTemplates.DateRange,
                ...
                React.createElement(Checkbox, {
                  defaultChecked:
                    item.selectedTemplate ===
                      BuiltinFilterTemplates.DateRange ||
                    item.selectedTemplate ===
                      BuiltinFilterTemplates.DateInterval
                      ? false
                      : item.isMulti,
                  disabled:
                    item.selectedTemplate ===
                  ...
                  options: [
                    {
                      ...
                      text: webPartStrings.PropertyPane.DataFilterCollection
                        .ANDOperator,
                    },
                    {
          ...
          },
          {
            id: "sortDirection",
            title:
              webPartStrings.PropertyPane.DataFilterCollection.SortDirection,
            type: this._customCollectionFieldType.custom,
  ...
  private async getVerticalsConnectionFields(): Promise<
    IPropertyPaneField<any>[]
  > {
    let verticalsConnectionFields: IPropertyPaneField<any>[] = [
      PropertyPaneToggle("useVerticals", {
        label:
          commonStrings.PropertyPane.ConnectionsPage
            .UseDataVerticalsWebPartLabel,
        checked: this.properties.useVerticals,
      }),
    ];

    if (this.properties.useVerticals) {
      verticalsConnectionFields.splice(
        1,
        0,
        PropertyPaneDropdown("verticalsDataSourceReference", {
          options: await this.dynamicDataService.getAvailableDataSourcesByType(
            ComponentType.SearchVerticals
          ),
          label:
            commonStrings.PropertyPane.ConnectionsPage
      ...
      if (this.properties.verticalsDataSourceReference) {
        ...
        if (this._verticalsSourceData) {
          const availableVerticals = DynamicPropertyHelper.tryGetValueSafe(
            this._verticalsSourceData
          );
          ...
          if (availableVerticals) {
            ...
            availableVerticals.verticalsConfiguration.forEach(
              (verticalConfiguration) => {
                if (
                  this.properties.selectedVerticalKeys.indexOf(
                    verticalConfiguration.key
                  ) !== -1
                ) {
            ...
            verticalsConnectionFields.push(
              new PropertyPaneAsyncCombo("selectedVerticalKeys", {
                availableOptions: availableVerticals.verticalsConfiguration
                  .filter((v) => !v.isLink)
                  .map((verticalConfiguration) => {
                    return {
                ...
                description:
                  webPartStrings.PropertyPane.ConnectionsPage
                    .LinkToVerticalLabelHoverMessage,
                label:
                  webPartStrings.PropertyPane.ConnectionsPage
                    .LinkToVerticalLabel,
  ...
  private async onTemplateUrlChange(value: string): Promise<string> {
    try {
      ...
      // Resolves an error if the file isn't a valid .htm or .html file
        ...
        !this.templateService.isValidTemplateFile(value, [
          ".html",
          ".htm",
          ".txt",
        ])
      ...
      return Text.format(
        webPartStrings.PropertyPane.LayoutPage.ErrorTemplateResolve,
        error
      );
    }
  }
  ...
  private async initTemplate(): Promise<void> {
    ...
    const selectedLayoutTemplateContent =
      this.availableLayoutDefinitions.filter((layout) => {
        return layout.key === this.properties.selectedLayoutKey;
      })[0].templateContent;
    ...
    if (
      ...
      if (this.properties.externalTemplateUrl) {
        ...
        this.templateContentToDisplay =
          await this.templateService.getFileContent(
            this.properties.externalTemplateUrl,
            FileFormat.Text
          );
      } else {
        this.templateContentToDisplay = this.properties.inlineTemplateContent
          ? this.properties.inlineTemplateContent
          : selectedLayoutTemplateContent;
      }
    } else {
  ...
  private async initializeWebPartServices(): Promise<void> {
    this.webPartInstanceServiceScope =
      this.context.serviceScope.startNewChild();
    this.templateService = this.webPartInstanceServiceScope.createAndProvide(
      TemplateService.ServiceKey,
      TemplateService
    );
    this.dynamicDataService = this.webPartInstanceServiceScope.createAndProvide(
      DynamicDataService.ServiceKey,
      DynamicDataService
    );
  ...
  private initializeProperties() {
    this.properties.selectedLayoutKey = this.properties.selectedLayoutKey
      ? this.properties.selectedLayoutKey
      : BuiltinLayoutsKeys.Vertical;
    this.properties.inlineTemplateContent = this.properties
      .inlineTemplateContent
      ? this.properties.inlineTemplateContent
      : "";
    this.properties.filterOperator = this.properties.filterOperator
      ? this.properties.filterOperator
      : FilterConditionOperator.OR;
    this.properties.layoutProperties = this.properties.layoutProperties
      ? this.properties.layoutProperties
      : {};
    this.properties.dataResultsDataSourceReferences = this.properties
      .dataResultsDataSourceReferences
      ? this.properties.dataResultsDataSourceReferences
      : [];
    ...
    /* Verticals */
    this.properties.useVerticals =
      this.properties.useVerticals !== undefined
        ? this.properties.useVerticals
        : false;
  ...
  private ensureDynamicDataSourcesConnection() {
    ...
    if (this.properties.dataResultsDataSourceReferences.length > 0) {
      this.properties.dataResultsDataSourceReferences.forEach((reference) => {
        const dataSourceDynamicProperty =
          new DynamicProperty<IDataResultSourceData>(
            this.context.dynamicDataProvider
          );
    ...
    if (this.properties.verticalsDataSourceReference) {
      if (!this._verticalsSourceData) {
        this._verticalsSourceData =
          new DynamicProperty<IDataVerticalSourceData>(
            this.context.dynamicDataProvider
          );
      }

      this._verticalsSourceData.setReference(
        this.properties.verticalsDataSourceReference
      );
  ...
  public async loadPropertyPaneResources(): Promise<void> {
    const { PropertyFieldCodeEditor, PropertyFieldCodeEditorLanguages } =
      await import(
   ...
   */
  private _initStaticFilters(
    filterResults: IDataFilterResult[],
      ...
      if (filterConfiguration.type === FilterType.StaticFilter) {
        ...
        if (
          filterResults.filter(
            (filterResult) =>
              filterResult.filterName === filterConfiguration.filterName
          ).length === 0
        ) {
   ...
   */
  private mergeFiltersDataFromSources(
    dynamicProperties: DynamicProperty<IDataResultSourceData>[]
  ): IDataResultSourceData {
    ...
    dynamicProperties.forEach((dataSourceDynamicProperty) => {
      const dataSourceData = DynamicPropertyHelper.tryGetValueSafe(
        dataSourceDynamicProperty
      );
      if (dataSourceData) {
        // 1. Concatenate all values from all connected results Web Parts
        allAvailableFieldsFromResults = allAvailableFieldsFromResults.concat(
          dataSourceData.availableFieldsFromResults
        );
        allAvailableFilters = allAvailableFilters.concat(
          dataSourceData.availablefilters
        );
        ...
        if (dataSourceData.handlebarsContext) {
          Object.keys(dataSourceData.handlebarsContext.helpers).forEach(
            (helperName) => {
              if (!this.templateService.Handlebars.helpers[helperName]) {
                this.templateService.Handlebars.registerHelper(
                  helperName,
                  dataSourceData.handlebarsContext.helpers[helperName]
                );
              }
            }
          );
    ...
    allAvailableFilters.forEach((filterResult) => {
      const mergedFilterIdx = allMergedFilters
        .map((m) => m.filterName)
        .indexOf(filterResult.filterName);

      if (mergedFilterIdx === -1) {
        allMergedFilters.push(filterResult);
      } else {
        let allMergedValues: IDataFilterResultValue[] = [];
        const allValues = allMergedFilters[mergedFilterIdx].values.concat(
          filterResult.values
        );

        // 3. Sum counts for similar value names
        allValues.forEach((value) => {
          const mergedValueIdx = allMergedValues
            .map((v) => v.name)
            .indexOf(value.name);
          ...
          if (mergedValueIdx === -1) {
            ...
            allMergedValues[mergedValueIdx].count =
              allMergedValues[mergedValueIdx].count + value.count;
          }
        });
    ...
    const sortedFilters = allMergedFilters.map((filter) => {
      ...
      const filterConfigurationIdx = this.properties.filtersConfiguration
        .map((configuration) => configuration.filterName)
        .indexOf(filter.filterName);
      if (filterConfigurationIdx !== -1) {
        const filterConfiguration =
          this.properties.filtersConfiguration[filterConfigurationIdx];
        if (filterConfiguration.sortBy === FilterSortType.ByCount) {
          sortByField = "count";
        }

        if (
          filterConfiguration.sortDirection === FilterSortDirection.Descending
        ) {
    ...
    dataResultsSourceData.availableFieldsFromResults = uniq(
      allAvailableFieldsFromResults
    );
